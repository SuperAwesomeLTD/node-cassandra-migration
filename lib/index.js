...
(function() {
  var FS, Q, _, applyMigration, cassandra, createVersionTable, debugMode, durations, getCassandraClient, getSchemaVersion, listMigrations, logDebug, logError, logInfo, migrate, moduleVersion, program, quietMode, readConfig, runQuery, runScript;
  ...
  runScript = function() {
    ...
    program.version(moduleVersion).usage('[options] <config_file>').option('-q, --quiet', 'Silence non-error output (default is false)').option('-d, --debug', 'Increase verbosity and error detail').option('-t, --target-version <version>', 'Maximum migration version to apply (default runs all migrations)').parse(process.argv);
    configFile = _(program.args).last();
    code = 1;
    cassandraClient = void 0;
    return readConfig(configFile).then(function(config) {
      var keyspace, ref, ref1, ref2;
      config.quiet = (ref = program.quiet) != null ? ref : config.quiet;
      config.debug = (ref1 = program.debug) != null ? ref1 : config.debug;
      config.targetVersion = (ref2 = program.targetVersion) != null ? ref2 : Number.MAX_VALUE;
